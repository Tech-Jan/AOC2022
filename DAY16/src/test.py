...
part1 = True  # set to true for Part 1, False for Part 2
if part1:
    maxtime = 30
else:
    maxtime = 26
data = [[re.findall(r'(?<=Valve\s)[A-Z]+', x[0])[0], int(re.findall(r'[0-9]+', x[0])[0]), re.findall(r'[A-Z]+', x[1])]
        for x in [y.split(';') for y in open('../input/testinput').read().split('\n')]]
    ...
    if len(p) == len(valves) + 3:
        ...
        for n in news:
            ...
            if n[0] + delta >= maxtime:
                for v in n[
                         2:-1]:  # finish out the time for all but the most recent node, since we couldn't get there in time
                    if v in valves:
